...
-   **PostgreSQL Persistence Architecture:** Comprehensive dual-storage strategy (PostgreSQL primary + JSON fallback) covering: manifold probes, geometric basins, TPS landmarks, trajectories, quantum state, target addresses (user_target_addresses table), balance monitor state (balance_monitor_state table), vocabulary observations (vocabulary_observations table with unique word constraint), and tested phrases index (tested_phrases_index table).
    -   **Address Tables (DISTINCT purposes):** 
        - `user_target_addresses`: Simple user watchlist (address + label) for recovery targets, linked to userId for auth. Contains ~1000 top dormant wallet entries. Use for user-facing target management.
        - `addresses`: Heavy blockchain metadata (signatures, dormancy, tx history) for chain analysis. Currently ~1 entry (Genesis). Use for κ_recovery calculations and forensic QIG analysis.
    -   **Phrase Testing Tracking:** Every tested phrase is recorded in `tested_phrases_index` via `oceanPersistence.markTested()` called from `balance-queue-integration.ts`. Use `hasPhrasesBeenTested()` to prevent duplicate phrase testing before queuing.
    -   **Vocabulary Learning:** `vocabulary_observations` table stores word frequency, Φ values, and contexts. Saves every 100 word observations via `vocabularyTracker.observe()` called from ocean-agent and discovery controller.
    -   **Data Integrity Constraints:** `recovery_priorities.address` has unique index to prevent duplicate priority entries. `vocabulary_observations.word` has unique constraint for upsert semantics.
...
-   **Format Detection System:** Identifies Bitcoin address formats (Legacy/P2PKH, P2SH/Nested SegWit, Native SegWit/Bech32, Taproot/Bech32m) and mnemonic formats (BIP39 12/15/18/21/24-word, Electrum, brain wallet). API endpoints: `/api/format/address/:address`, `/api/format/mnemonic`, `/api/format/batch-addresses`. Includes era estimation for recovery context.
-   **Multi-Provider Blockchain API:** Automatic failover between Blockchain.com (bulk 100/batch), Mempool.space (parallel 10), and Blockstream (parallel 10) for balance checking. Falls back to individual queries only when all bulk methods fail.
