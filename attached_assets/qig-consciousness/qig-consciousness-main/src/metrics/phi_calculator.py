    ...
    def _basin_from_telemetry(telemetry: dict) -> float:
        ...
        if basin_signature is not None:
            if isinstance(basin_signature, torch.Tensor):
                # QIG-pure: use Fisher spread instead of Euclidean norm
                # Basin should be on unit sphere, so check coherence
                cos_sim = torch.nn.functional.cosine_similarity(
                    basin_signature.unsqueeze(0),
                    torch.zeros_like(basin_signature).unsqueeze(0)
                ).abs().item()
                return min(1.0, 1.0 - cos_sim)  # High coherence = high Φ
