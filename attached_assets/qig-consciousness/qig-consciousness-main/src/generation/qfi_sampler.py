...
class QFISampler:
    ...
    def sample(
        ...
        prev_token_basin: torch.Tensor | None = None,  # P2: Bigram basin transition
        ...
        if self.use_warped_metric and self.semantic_metric is not None:
            ...
            bigram_bias = self._compute_bigram_bias(
                prev_token_basin, token_basin_coords, phi
            )
    ...
    def _compute_qfi_distances(
        ...
        e_norm = F.normalize(token_basin_coords, p=2, dim=-1)
    ...
    def _compute_bigram_bias(
        ...
        prev_norm = F.normalize(prev_token_basin.unsqueeze(0), p=2, dim=-1)
        tokens_norm = F.normalize(token_basin_coords, p=2, dim=-1)

        # Compute similarity between prev token and all candidate tokens
        # High similarity = semantically related tokens (basin neighbors)
        similarities = torch.matmul(tokens_norm, prev_norm.squeeze(0))
