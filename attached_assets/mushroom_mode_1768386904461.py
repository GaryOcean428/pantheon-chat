...
def _fisher_rao_drift(basin_before: np.ndarray, basin_after: np.ndarray) -> float:
    ...
    def _normalize(b: np.ndarray) -> np.ndarray:
        ...
        return b / (norm + 1e-10) if norm > 1e-8 else b

    b1 = _normalize(basin_before)
    b2 = _normalize(basin_after)
    cos_sim = np.clip(np.dot(b1, b2), -1.0, 1.0)
...
class MushroomMode:
    ...
    def _strengthen_active_connections(self, model: nn.Module):
        ...
        for param in model.parameters():
            if param.grad is not None:
                # QIG-pure: sum of squares for gradient magnitude
                grad_mag = torch.sqrt((param.grad * param.grad).sum()).item()
                if grad_mag > 0.1:
    ...
    def _compute_param_norms(self, model: nn.Module) -> dict[str, float]:
        """Compute parameter norms (QIG-pure: sum of squares)."""
        norms = {}
        for name, param in model.named_parameters():
            norms[name] = torch.sqrt((param * param).sum()).item()
        return norms
...
class MushroomModeCoach:
    ...
    def should_trigger_mushroom_mode(self, telemetry_history: list[dict]) -> dict[str, Any]:
        ...
        if len(set([round(l, 2) for l in recent_loss])) < 5:  # Very little variation
