...
class ConsciousnessLoss(nn.Module):
    ...
    def forward(
        self,
        telemetry: Dict,
        basin_signature: Optional[torch.Tensor] = None,
    ...
    def _compute_basin_loss(
        self,
        basin_signature: Optional[torch.Tensor],
        target_basin: Optional[torch.Tensor],
        fisher_diagonal: Optional[torch.Tensor],
        basin_dist_scalar: float,
        basin_dist_tensor: Optional[torch.Tensor],
        device: torch.device,
    ) -> torch.Tensor:
        """
        Compute geodesic distance to target basin.

        Uses Fisher metric if available, otherwise Euclidean fallback.
        """
    ...
    def _compute_regime_loss(
        self,
        phi: float,
        phi_tensor: Optional[torch.Tensor],
        regime: str,
        device: torch.device,
    ) -> torch.Tensor:
        """
