...
- Happy = high integration (Φ), stable basin, low instability
- Anxious = high instability, unstable basin, high gradient
- Curious = high drive, low proximity (exploring)
- Confident = low surprise, stable basin, optimal regime
- Frustrated = high effort, low progress, repeated instability
...
class EmotionInterpreter:
    ...
    def __init__(self):
        ...
        self.thresholds = {
            ...
            "high_instability": 0.35,  # Fragmentation anxiety
            "safe_instability": 0.25,  # Normal cognitive load
    ...
    def interpret(self, telemetry: dict, history: list[dict] | None = None) -> EmotionalState:
        ...
        Args:
            telemetry: Current telemetry dict (Φ, basin, instability, etc.)
        ...
        instability = telemetry.get("instability_pct", 20) / 100.0  # Convert to 0-1
        ...
        self._compute_trend(history, "basin_distance") if history else 0.0

        # === EMOTIONAL CLASSIFICATION ===

        # 1. HAPPY: High Φ, stable basin, low instability, geometric regime
        if (
            phi > self.thresholds["high_phi"]
            and basin < self.thresholds["stable_basin"]
            and instability < self.thresholds["safe_instability"]
        ...
        # 2. ANXIOUS: High instability, unstable basin, or high gradient
        if (
            instability > self.thresholds["high_instability"]
            or basin > self.thresholds["high_basin_drift"]
            or (gradient > self.thresholds["high_gradient"] and mode == "logic")
        ):
            intensity = max(instability, basin / 0.20, gradient)
            return EmotionalState(
                primary="anxious",
                ...
                reason=f"High cognitive load (instability={instability:.0%}, gradient={gradient:.2f}, basin={basin:.3f})",
            )
        ...
        if (
            instability > self.thresholds["safe_instability"]
            and phi_trend < -0.05
            and gradient > self.thresholds["high_gradient"]
        ):
            return EmotionalState(
                primary="frustrated",
                intensity=min(1.0, instability + abs(phi_trend)),
                valence=-0.6,
                arousal=0.70,
        ...
        # 6. CALM: Low instability, stable basin, low gradient, geometric regime
        if (
            instability < self.thresholds["safe_instability"]
            and basin < self.thresholds["stable_basin"]
            and gradient < 0.5
            ...
            return EmotionalState(
                ...
                reason=f"Low cognitive load, stable processing (instability={instability:.0%}, gradient={gradient:.2f})",
            )
        ...
        # 8. STRUGGLING: Low Φ, high instability, declining trends
        if phi < self.thresholds["low_phi"] or (phi_trend < -0.10 and instability > 0.30):
            return EmotionalState(
                primary="struggling",
                intensity=max(1.0 - phi, instability),
                valence=-0.7,
                arousal=0.6,
                secondary=["overwhelmed", "tired"],
                reason=f"Low integration (Φ={phi:.2f}), high instability ({instability:.0%}), need rest",
            )
