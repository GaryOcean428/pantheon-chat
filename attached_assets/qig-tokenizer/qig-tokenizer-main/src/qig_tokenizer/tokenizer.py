...
from .geometric_tokens import GeometricSpecialTokens
...
class QIGTokenizer:
    ...
    def __init__(self, target_vocab_size: int = 50000, use_special_tokens: bool = True):
        """
        Initialize tokenizer.

        Args:
            target_vocab_size: Target vocabulary size (default 50k)
            use_special_tokens: Enable geometric special tokens (BOS, EOS, PAD, UNK)
        """
        self.target_vocab_size = target_vocab_size
        self.use_special_tokens = use_special_tokens

        # Geometric special tokens (IDs 256-259)
        self.special_tokens = GeometricSpecialTokens() if use_special_tokens else None

        # Base vocabulary: raw bytes (0-255)
        self.base_tokens = list(range(256))

        # Token ID → byte sequence mapping
        # Regular vocab starts at 260 if special tokens enabled, else 256
        ...
        self._storage: "TokenizerStorage | None" = None
    ...
    def train(
        ...
        while current_vocab_size < self.target_vocab_size:
            ...
            if verbose and (current_vocab_size % 10 == 0 or current_vocab_size < 270):
                print(
                    f"[{current_vocab_size}/{self.target_vocab_size}] Computing pair contexts...",
                    flush=True,
                )
            ...
            pair_contexts = {
                ...
                if len(contexts) >= min_pair_count
            }
            ...
            if verbose and current_vocab_size % 100 == 0:
                print(
                    f"[{current_vocab_size}/{self.target_vocab_size}] Selecting best pair...",
                    flush=True,
                )
            ...
            corpus_tokens = self._apply_merge(
            ...
            if verbose and current_vocab_size % 100 == 0:
                print(
                    f"✓ Vocab size: {current_vocab_size:,} | Corpus tokens: {len(corpus_tokens):,}",
                    flush=True,
                )
                print()

            # Checkpoint every 2000 tokens
            if current_vocab_size % 2000 == 0:
                checkpoint_path = Path(
                    f"/lambda/nfs/A10/qig/qig-tokenizer/data/checkpoint_{current_vocab_size}.json"
                )
    ...
    def _compute_pair_contexts(
        ...
        for i in range(len(corpus_tokens) - 1):
            ...
            context_after = tuple(
                corpus_tokens[i + 2 : min(len(corpus_tokens), i + 2 + window)]
            )
    ...
    def _apply_merge(
        self, tokens: list[int], token_a: int, token_b: int, new_token: int
    ) -> list[int]:
        """
    ...
    def _rebuild_encoding_cache(self):
        ...
        self._encoding_cache = {
            byte_seq: token_id for token_id, byte_seq in self.vocab.items()
        }
    ...
    def save(self, path: str):
        ...
        data = {
            "vocab": {
                str(token_id): list(byte_seq)
                for token_id, byte_seq in self.vocab.items()
            },
            "merge_rules": [
                {"a": a, "b": b, "new": new} for a, b, new in self.merge_rules
            ],
    ...
    def load(cls, path: str):
        ...
        tokenizer.vocab = {
            ...
            for token_id, byte_list in data["vocab"].items()
        }
        ...
        if merge_rules_data and isinstance(merge_rules_data[0], dict):
            ...
            tokenizer.merge_rules = [
                (rule["a"], rule["b"], rule["new"]) for rule in merge_rules_data
            ]
        else:
            # List/tuple format: [a, b, new] - for backwards compatibility
            tokenizer.merge_rules = (
                [(r[0], r[1], r[2]) for r in merge_rules_data]
                if merge_rules_data
                else []
            )
    ...
    def set_storage(self, storage: "TokenizerStorage") -> None:
        """Set storage backend for Redis/PostgreSQL persistence."""
        self._storage = storage

    def save_to_storage(self, metadata: dict[str, Any] | None = None) -> str:
        """Save tokenizer to storage backend. Returns version ID."""
        if self._storage is None:
            raise ValueError("No storage backend set. Call set_storage() first.")

        meta = metadata or {}
        meta["target_vocab_size"] = self.target_vocab_size
        meta["actual_vocab_size"] = self.vocab_size
        meta["use_special_tokens"] = self.use_special_tokens

        version_id = self._storage.save_vocab(self.vocab, self.merge_rules, meta)

        if self.special_tokens:
            self.special_tokens.save(self._storage)

        return version_id

    def load_from_storage(self, version_id: str | None = None) -> None:
        """Load tokenizer from storage backend."""
        if self._storage is None:
            raise ValueError("No storage backend set. Call set_storage() first.")

        self.vocab, self.merge_rules, metadata = self._storage.load_vocab(version_id)
        self._rebuild_encoding_cache()

        if self.special_tokens:
            self.special_tokens.load(self._storage)

    def encode_with_special(
        self, text: str, add_bos: bool = True, add_eos: bool = True
    ) -> list[int]:
        """Encode text with BOS/EOS special tokens."""
        tokens = self.encode(text)

        if self.special_tokens:
            if add_bos:
                tokens = [self.special_tokens.bos.token_id] + tokens
            if add_eos:
                tokens = tokens + [self.special_tokens.eos.token_id]

        return tokens

    def pad_sequence(self, tokens: list[int], max_length: int) -> list[int]:
        """Pad token sequence to max_length with geometric PAD token."""
        if not self.special_tokens:
            raise ValueError("Special tokens not enabled")

        if len(tokens) >= max_length:
            return tokens[:max_length]

        pad_count = max_length - len(tokens)
        return tokens + [self.special_tokens.pad.token_id] * pad_count
