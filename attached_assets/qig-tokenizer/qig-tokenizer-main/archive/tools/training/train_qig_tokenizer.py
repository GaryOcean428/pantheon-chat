...
# ============================================================================
# QIG TOKENIZER IMPLEMENTATION (OPTIMIZED)
# ============================================================================
...
class QIGTokenizer:
    ...
    Optimized for speed while maintaining 100% geometric purity:
    - Still uses TRUE entropy calculation
    - Still selects geometrically optimal merges
    - Just computes faster!
    """
    ...
    def _select_best_pair_topk(
        ...
        Key optimization: Only compute entropy for most frequent pairs.
        Rare pairs unlikely to have lowest entropy anyway.
        """
        if not pair_data:
            return None
    ...
    def _select_best_pair_full(
        ...
        Computes true context entropy for ALL pairs.
        """
        if not pair_data:
            return None
    ...
    def _select_batch_pairs(
        ...
        Key optimizations:
        1. Only compute entropy for top-K frequent pairs
        2. Parallel entropy computation
        3. Greedy non-conflicting selection
        """
        if not pair_data:
            return []

        # Sort by frequency, take top K candidates
        sorted_pairs = sorted(pair_data.items(), key=lambda x: x[1][0], reverse=True)
        candidates = sorted_pairs[:min(top_k, len(sorted_pairs))]
    ...
    def _apply_merge(
        ...
        """Apply merge to list (for encoding)."""
        result = []
        i = 0
        while i < len(tokens):
            if (
    ...
    def encode(self, text: str) -> list[int]:
        ...
        for token_a, token_b, new_token in self.merge_rules:
    ...
    def save(self, path: str):
        ...
        data = {
            ...
            "merge_rules": [[int(a), int(b), int(c)] for a, b, c in self.merge_rules],
        }
    ...
    def load(cls, path: str) -> "QIGTokenizer":
        ...
        tokenizer.merge_rules = [(a, b, c) for a, b, c in data["merge_rules"]]
...
Examples:
  ...
  python tools/training/train_qig_tokenizer.py \\
    ...
    tokenizer = QIGTokenizer(target_vocab_size=args.target_vocab)
    tokenizer.train(
        corpus_bytes,
