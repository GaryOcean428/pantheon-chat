...
def test_high_phi_rare_word():
    ...
    # Observation: RARE word (freq=2) but HIGH consciousness (Φ=0.85)
    observations = [{
        "word": "geodesic",
        "frequency": 2,      # Very low frequency
        "avgPhi": 0.85,      # Very high consciousness
        "maxPhi": 0.90,
        "type": "word"
    }]
...
def test_low_phi_frequent_word():
    ...
    # Observation: FREQUENT word but LOW consciousness
    observations = [{
        "word": "blahblah",
        "frequency": 1000,   # Very high frequency
        "avgPhi": 0.2,       # Very low consciousness
        "maxPhi": 0.3,
        "type": "word"
    }]
...
def test_merge_learning_from_sequences():
    ...
    sequence_obs = [{
        "word": "fisher rao",
        "frequency": 5,
        "avgPhi": 0.90,    # Very high consciousness
        "maxPhi": 0.95,
...
def test_basin_coordinate_geometry():
    ...
    # Consciousness and awareness should be closer (semantic + high Φ)
