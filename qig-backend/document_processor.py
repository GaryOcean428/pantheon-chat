...
try:
    ...
    try:
        ...
        print(
            "[DocumentProcessor] PyPDF2/pypdf not available - PDF extraction disabled"
        )
...
def extract_pdf_text():
    ...
    try:
        ...
        return (
            jsonify(
                {"error": "PDF extraction failed", "message": "Internal server error"}
            ),
            500,
        )
...
def ingest_knowledge():
    ...
    try:
        ...
        return jsonify(
            {
                ...
                "knowledge_id": doc_id,
                "basin_coords": (
                    basin_list[:8] if basin_list else []
                ),  # Return first 8 dims for brevity
                "metadata": metadata,
                "timestamp": datetime.now().isoformat(),
            }
        )
        ...
        return (
            jsonify({"error": "Ingestion failed", "message": "Internal server error"}),
            500,
        )
    ...
    try:
        ...
        for doc_id, doc in rag.documents.items():
            doc_data = {
                ...
                "kappa": doc.kappa,
                "created_at": (
                    datetime.fromtimestamp(doc.timestamp).isoformat()
                    if doc.timestamp
                    else None
                ),
            }
        ...
        return (
            jsonify({"error": "List failed", "message": "Internal server error"}),
            500,
        )
    ...
    return jsonify(
        {
            ...
            "document": {
                ...
                "regime": doc.regime,
                "basin_coords": (
                    doc.basin_coords.tolist()[:8]
                    if doc.basin_coords is not None
                    else []
                ),
                "created_at": (
                    datetime.fromtimestamp(doc.timestamp).isoformat()
...
def search_knowledge():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "results": results, "total": len(results), "query": query}
        )
    except Exception:
        logger.exception("Search failed")
        return (
            jsonify({"error": "Search failed", "message": "Internal server error"}),
            500,
        )
...
def knowledge_stats():
    ...
    try:
        ...
        return (
            jsonify({"error": "Stats failed", "message": "Internal server error"}),
            500,
        )
...
def register_document_routes(app):
    ...
    print(
        "[DocumentProcessor] Registered document routes at /api/documents/* and /api/ocean/knowledge/*"
    )
