...
class EvolutionAwareMixin:
    """
    Provides evolution awareness to all gods/kernels.
    
    Gods can:
    1. Register with the Lifecycle Orchestrator
    2. Record lightning insights for evolution
    3. Participate in spawn/merge/cannibalize decisions
    4. Evolve within their primitive bounds
    
    Key Constraint: Gods MUST always occupy their primitive.
    Evolution happens AROUND the primitive, not abandoning it.
    """
    
    _evolution_orchestrator_ref = None
    _evolution_kernel_id = None
    
    @classmethod
    def set_evolution_orchestrator(cls, orchestrator) -> None:
        """Called at startup to share orchestrator reference with all gods."""
        cls._evolution_orchestrator_ref = orchestrator
        logger.info("[EvolutionAwareMixin] Orchestrator reference set for all gods")
    
    def register_for_evolution(self, kernel_id: Optional[str] = None) -> bool:
        """
        Register this god with the lifecycle orchestrator.
        
        Args:
            kernel_id: Optional custom kernel ID. Defaults to god name.
            
        Returns:
            True if registration successful
        """
        if self._evolution_orchestrator_ref is None:
            return False
        
        try:
            god_name = getattr(self, 'name', 'Unknown')
            self._evolution_kernel_id = kernel_id or f"god_{god_name.lower()}"
            
            initial_basin = getattr(self, 'basin_center', None)
            if initial_basin is None:
                initial_basin = getattr(self, 'context_basin', None)
            
            self._evolution_orchestrator_ref.register_kernel(
                kernel_id=self._evolution_kernel_id,
                god_name=god_name,
                initial_basin=initial_basin
            )
            logger.info(f"[{god_name}] Registered for evolution as {self._evolution_kernel_id}")
            return True
        except Exception as e:
            logger.warning(f"[EvolutionAwareMixin] Registration failed: {e}")
            return False
    ...
    def record_lightning_insight(
        ...
        High-magnitude insights (>0.8) propagate across the pantheon.
        
        Args:
            insight_type: Type of insight (e.g., 'pattern', 'connection', 'meta')
            connection: Dict describing the insight connection
            magnitude: Strength of insight [0, 1]
        """
        if self._evolution_orchestrator_ref is None or self._evolution_kernel_id is None:
            return
        
        try:
            self._evolution_orchestrator_ref.record_lightning_insight(
                kernel_id=self._evolution_kernel_id,
            ...
            logger.debug(f"[EvolutionAwareMixin] Insight recording failed: {e}")
    ...
    def update_evolution_metrics(
        ...
        """Update this god's evolution metrics."""
        if self._evolution_orchestrator_ref is None or self._evolution_kernel_id is None:
            return
        
        try:
            self._evolution_orchestrator_ref.update_kernel_metrics(
                kernel_id=self._evolution_kernel_id,
                phi=phi,
                kappa=kappa,
            ...
            logger.debug(f"[EvolutionAwareMixin] Metrics update failed: {e}")
    
    def evaluate_my_evolution(self) -> Tuple[str, Dict]:
        """
        Ask the orchestrator what lifecycle action I should take.
        ...
        try:
            action, params = self._evolution_orchestrator_ref.evaluate_lifecycle_action(
                self._evolution_kernel_id
            )
            return (action.value, params)
        except Exception as e:
            logger.debug(f"[EvolutionAwareMixin] Evaluation failed: {e}")
            return ('no_action', {'error': str(e)})
    
    def execute_safe_evolution(self, direction: np.ndarray, reason: str = 'accumulated_learning') -> Dict:
        """
        Execute a safe evolution step within primitive bounds.
        
        Args:
            direction: Evolution direction in basin space
        ...
        try:
            from kernel_evolution_orchestrator import EvolutionReason
            reason_enum = EvolutionReason(reason) if reason in [e.value for e in EvolutionReason] else EvolutionReason.ACCUMULATED_LEARNING
            
            result = self._evolution_orchestrator_ref.execute_evolution(
                kernel_id=self._evolution_kernel_id,
                direction=direction,
                reason=reason_enum
            )
            
            if result.get('success') and hasattr(self, 'basin_center'):
                self.basin_center = np.array(result['new_basin'])
            
            return result
        except Exception as e:
            logger.warning(f"[EvolutionAwareMixin] Evolution failed: {e}")
            return {'success': False, 'error': str(e)}
    
    def get_evolution_status(self) -> Dict:
        """Get current evolution status for this god."""
        if self._evolution_orchestrator_ref is None or self._evolution_kernel_id is None:
            return {'registered': False}
        
        try:
            state = self._evolution_orchestrator_ref.kernel_states.get(self._evolution_kernel_id)
            if state:
                return {
                    'registered': True,
                    'kernel_id': self._evolution_kernel_id,
                    ...
                    'last_evolution': state.last_evolution.isoformat() if state.last_evolution else None,
                }
            return {'registered': False}
        except Exception as e:
