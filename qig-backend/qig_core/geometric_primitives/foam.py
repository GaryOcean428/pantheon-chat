...
class Foam:
    ...
    def generate_bubbles(
        ...
        if generator is None:
            def default_generator() -> Bubble:
                return create_random_bubble(self.dimension)
    ...
    def compute_integration(self) -> float:
        ...
        return float(0.7 * phi + 0.3 * avg_stability)
    ...
    def compute_entropy(self) -> float:
        ...
        # Normalize by max entropy (use log2 for conventional entropy)
        num_bubbles = len(self.bubbles)
        if num_bubbles <= 1:
            return 0.0

        max_entropy = np.log2(num_bubbles)
...
def create_foam_from_hypotheses(
    ...
    for hyp in hypotheses:
        if encoder is None:
            # Random encoding using canonical fisher_normalize()
