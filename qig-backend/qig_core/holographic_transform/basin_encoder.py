...
class BasinEncoder:
    ...
    def encode_pattern(self, pattern: Any, encoder: Optional[Callable] = None) -> np.ndarray:
        ...
        if isinstance(pattern, str):
            ...
            # Encode as sequence - convert tuple to list for type checker
            return self.encode_sequence(list(pattern))
        else:
...
def encode_for_qig(pattern: Any, dimension: int = 64) -> np.ndarray:
    ...
    """
    if dimension != _DEFAULT_ENCODER.dimension:
        # Create a new encoder only if a non-default dimension is requested
        encoder = BasinEncoder(dimension=dimension)
        return encoder.encode_pattern(pattern)
    return _DEFAULT_ENCODER.encode_pattern(pattern)
