...
This script should be run as a pre-commit hook to ensure
...
try:
    ...
    print(f"Error importing qig_types: {e}", file=sys.stderr)
    print("Make sure qig_types.py exists and pydantic is installed", file=sys.stderr)
    sys.exit(1)

def generate_typescript_types():
    """
    Generate TypeScript types from Pydantic models.
    Manual generation since pydantic-to-typescript requires Node.js.
    """
...
"""
    ...
    # Write to shared/types directory
    output_path = Path(__file__).parent.parent / "shared" / "types" / "qig-generated.ts"
...
if __name__ == "__main__":
    try:
        output_path = generate_typescript_types()
        print("\n✅ Type generation complete!")
        print(f"   Import in TypeScript: import {{ ConsciousnessMetrics }} from '@shared/types/qig-generated';")
    except Exception as e:
        print(f"\n❌ Type generation failed: {e}", file=sys.stderr)
        sys.exit(1)
