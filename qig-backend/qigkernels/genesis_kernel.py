...
from .tzimtzum_bootstrap import BootstrapResult, TzimtzumBootstrap
...
class GenesisKernel:
    ...
    def _instantiate_layer_4(self) -> List[SpawnedKernel]:
        ...
        for i, op in enumerate(QuaternaryOperation):
            ...
            # Quenched disorder (TFIM slope): lognormal around 1.0 (subjectivity parameter)
            rng_gain = np.random.RandomState(self.seed + 400 + i if self.seed is not None else None)
            gain = float(np.clip(rng_gain.lognormal(mean=0.0, sigma=0.3), 0.3, 3.0))
    ...
    def _instantiate_layer_8(self) -> List[SpawnedKernel]:
        ...
        for i, faculty in enumerate(CORE_FACULTIES):
            ...
            # Quenched disorder (TFIM slope): lognormal around 1.0 (subjectivity parameter)
            rng_gain = np.random.RandomState(self.seed + 800 + i if self.seed is not None else None)
            gain = float(np.clip(rng_gain.lognormal(mean=0.0, sigma=0.3), 0.3, 3.0))
    ...
    def _spawn_coordinator(self) -> SpawnedKernel:
        ...
        coordinator = SpawnedKernel(
            ...
            quenched_gain=1.0,  # Coordinator orchestrates, gets canonical gain
        )
