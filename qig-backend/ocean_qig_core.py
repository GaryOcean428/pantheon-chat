...
def get_kernel_capabilities():
    """
    Get all registered kernel capabilities.
    
    Returns summary of all kernels and their capabilities.
    """
    try:
        from kernel_capability_registry import get_capability_registry, register_all_olympians
        
        registry = get_capability_registry()
        
        # Auto-register Olympians if registry is empty
        stats = registry.get_registry_stats()
        if stats['total_kernels'] == 0:
            register_all_olympians()
...
@app.route('/capabilities/<kernel_id>', methods=['GET'])
def get_kernel_capability(kernel_id: str):
    """
    Get capabilities for a specific kernel.
    """
    try:
        from kernel_capability_registry import get_capability_registry, register_olympian
        
        registry = get_capability_registry()
        
        # Try to register if not found
        profile = registry.get_kernel(kernel_id)
        if not profile:
            register_olympian(kernel_id)
            profile = registry.get_kernel(kernel_id)
...
@app.route('/capabilities/search', methods=['GET'])
def search_capabilities():
    """
    Search for kernels by capability category or domain.
    
    Query params:
        category: Capability category (generation, search, analysis, etc.)
        domain: Domain to search for
    """
    try:
        from kernel_capability_registry import get_capability_registry
        ...
        if category:
            kernel_ids = registry.find_kernels_by_capability(category)
            results.extend(kernel_ids)
        
        if domain:
            kernel_id = registry.get_kernel_for_domain(domain)
            if kernel_id and kernel_id not in results:
                results.append(kernel_id)
        
        return jsonify({
            'success': True,
            'kernels': results,
            'query': {'category': category, 'domain': domain}
        })
    except ImportError:
        return jsonify({
            'success': False,
