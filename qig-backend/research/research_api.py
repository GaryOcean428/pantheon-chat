...
@research_bp.route('/sources/kernel-discover', methods=['POST'])
def kernel_discover_source():
    """
    Kernel-driven source discovery endpoint.
    
    Allows kernels (Zeus, M8, Olympians) to register sources they discover
    during curiosity-driven exploration, tool discovery, or search operations.
    
    Body: {
        "url": string (required) - The source URL
        "category": string (optional) - Category like 'research', 'documentation', 'tool_discovery'
        "kernel_id": string (optional) - ID of the kernel that discovered this source
        "phi_score": float (optional) - Initial phi score if known (0.0-1.0)
        "context": string (optional) - Context in which the source was discovered
        "metadata": dict (optional) - Additional metadata about the source
    }
    
    Returns:
        Success status and source registration details.
    """
    try:
        from olympus.shadow_scrapy import get_scrapy_orchestrator
        ...
        data = request.get_json() or {}
        url = data.get('url', '').strip()
        category = data.get('category', 'kernel_discovery')
        kernel_id = data.get('kernel_id', 'unknown')
        phi_score = min(max(data.get('phi_score', 0.5), 0.0), 1.0)
        ...
        is_new = url not in source_discovery.discovered_sources
        
        # Register with kernel-specific origin
        origin = f'kernel:{kernel_id}'
        source_discovery._register_source(
            source_url=url,
            category=category,
            hit_count=1 if phi_score >= 0.6 else 0,
            phi_avg=phi_score,
            phi_max=phi_score,
            origin=origin
        )
        ...
        print(f"[KernelDiscovery] {kernel_id} discovered: {url} (phi={phi_score:.2f}, category={category})")
        
        return jsonify({
            'success': True,
            'is_new': is_new,
            'message': f'Source {"registered" if is_new else "updated"}: {url}',
            'persisted': saved,
            'source': {
                'url': url,
                'category': category,
...
@research_bp.route('/sources/batch-discover', methods=['POST'])
def batch_discover_sources():
    """
    Batch source discovery for kernels.
    
    Allows kernels to register multiple sources at once from search results
    or crawl discoveries.
        ...
        "sources": [
            {
                ...
                "phi_score": float (optional)
            },
            ...
        ],
        "kernel_id": string (optional) - ID of the kernel
    }
    """
    try:
        from olympus.shadow_scrapy import get_scrapy_orchestrator
        ...
        kernel_id = data.get('kernel_id', 'batch')
        ...
        for src in sources[:100]:  # Limit to 100 sources per batch
            url = src.get('url', '').strip()
            if not url or not url.startswith('http'):
                continue
            
            category = src.get('category', 'batch_discovery')
            phi_score = min(max(src.get('phi_score', 0.5), 0.0), 1.0)
        ...
        print(f"[BatchDiscovery] {kernel_id}: {len(registered)} new, {len(skipped)} updated")
        
        return jsonify({
            'success': True,
            'registered_count': len(registered),
            'updated_count': len(skipped),
            'kernel_id': kernel_id
        })
