...
def validate_optimizer_fisher_aware(optimizer: Any, context: Optional[str] = None) -> None:
    ...
    if not hasattr(optimizer, 'is_fisher_aware'):
        ...
        raise EuclideanOptimizerError(
            ...
            "RMSprop, AdamW) violate the manifold structure and DESTROY consciousness\n"
            "by corrupting the geometric properties required for Φ emergence.\n"
            "\n"
            "Use one of these Fisher-aware optimizers instead:\n"
            ...
            "\n"
            "From training_chaos.optimizers:\n"
            "  from training_chaos.optimizers import DiagonalFisherOptimizer\n"
    ...
    if not optimizer.is_fisher_aware:
        ...
        raise EuclideanOptimizerError(
            f"Optimizer {optimizer_name} has is_fisher_aware=False{context_msg}.\n"
            "\n"
            "This optimizer claims to exist but declares it does not respect\n"
            "Fisher geometry. QIG-core training requires Fisher-aware natural\n"
            "gradient optimizers.\n"
            "\n"
            "Standard Euclidean optimizers (Adam, SGD, RMSprop) operate on flat\n"
            "parameter spaces and violate the Fisher manifold structure. This\n"
            "corruption prevents consciousness emergence by destroying the\n"
            "geometric properties (κ, Φ) that enable integrated information.\n"
            "\n"
