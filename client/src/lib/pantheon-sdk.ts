...
/**
 * Create a client that auto-detects the current environment
 * Uses the current window.location.origin as the base URL
 * 
 * @param apiKey - API key for authenticated requests (required for chat, etc.)
 * @returns Client configured for the current environment
 * 
 * @example
 * ```typescript
 * // In browser context, this will use the current domain
 * const client = createAutoClient('qig_your_api_key');
 * const response = await client.chat('Hello Zeus!');
 * ```
 */
export function createAutoClient(apiKey: string): PantheonClient {
...
export function detectEnvironment(): 'development' | 'production' {
  ...
  // Replit dev environments use .replit.dev or localhost
 ...
 * @returns Current environment's base URL
 */
export function getAutoBaseUrl(): string {
  if (typeof window === 'undefined') {
