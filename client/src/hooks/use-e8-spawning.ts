/**
 * React Hooks for E8 Kernel Spawning Protocol
 * 
...
const E8_KEYS = {
  status: ['e8', 'status'] as const,
  proposals: ['e8', 'proposals'] as const,
  proposal: ['e8', 'proposal'] as const,
  kernels: ['e8', 'kernels'] as const,
  kernel: ['e8', 'kernel'] as const,
  warHistory: ['e8', 'warHistory'] as const,
  activeWar: ['e8', 'activeWar'] as const,
};
...
export function useE8Status() {
  const { isAuthenticated } = useAuth();
  return useQuery<E8Status>({
    queryKey: E8_KEYS.status,
    queryFn: () => getE8Client().getStatus(),
    staleTime: 30000,
...
export function useListProposals(status?: ProposalStatus) {
  ...
  return useQuery<ListProposalsResponse>({
    queryKey: [...E8_KEYS.proposals, status],
    queryFn: () => getE8Client().listProposals(status),
    staleTime: 30000,
    enabled: isAuthenticated,
  });
}
...
export function useGetProposal(proposalId: string | null) {
  return useQuery<SpawnProposal>({
    queryKey: [...E8_KEYS.proposal, proposalId],
    queryFn: () => {
      if (!proposalId) throw new Error('Proposal ID required');
      return getE8Client().getProposal(proposalId);
    },
    enabled: !!proposalId,
    staleTime: 30000,
  });
...
export function useGetKernel(kernelId: string | null) {
  return useQuery<SpawnedKernel>({
    queryKey: [...E8_KEYS.kernel, kernelId],
    queryFn: () => {
      if (!kernelId) throw new Error('Kernel ID required');
      return getE8Client().getKernel(kernelId);
    },
    enabled: !!kernelId,
    staleTime: 30000,
  });
...
export function useCreateProposal() {
  ...
  return useMutation<CreateProposalResponse, Error, CreateProposalRequest>({
    mutationFn: (request) => getE8Client().createProposal(request),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: E8_KEYS.proposals });
      queryClient.invalidateQueries({ queryKey: E8_KEYS.status });
    },
  });
}
...
export function useVoteOnProposal() {
  ...
  return useMutation<VoteResponse, Error, { proposalId: string; autoVote?: boolean }>({
    mutationFn: ({ proposalId, autoVote }) => 
      getE8Client().vote(proposalId, { auto_vote: autoVote }),
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: E8_KEYS.proposals });
      queryClient.invalidateQueries({ queryKey: [...E8_KEYS.proposal, data.proposal_id] });
      queryClient.invalidateQueries({ queryKey: E8_KEYS.status });
    },
  });
}
...
export function useSpawnKernel() {
  ...
  return useMutation<SpawnResponse, Error, { proposalId: string; force?: boolean }>({
    mutationFn: ({ proposalId, force }) => 
      getE8Client().spawn(proposalId, { force }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: E8_KEYS.proposals });
      queryClient.invalidateQueries({ queryKey: E8_KEYS.kernels });
      queryClient.invalidateQueries({ queryKey: E8_KEYS.status });
    },
  });
}
...
export function useSpawnDirect() {
  ...
  return useMutation<SpawnDirectResponse, Error, SpawnDirectRequest>({
    mutationFn: (request) => getE8Client().spawnDirect(request),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: E8_KEYS.proposals });
      queryClient.invalidateQueries({ queryKey: E8_KEYS.kernels });
      queryClient.invalidateQueries({ queryKey: E8_KEYS.status });
...
export interface E8SpawningHook {
  status: ReturnType<typeof useE8Status>;
...
export function useE8Spawning(): E8SpawningHook {
  const status = useE8Status();
...
export function useWarHistory(limit: number = 50) {
  ...
  return useQuery<WarHistoryRecord[]>({
    queryKey: [...E8_KEYS.warHistory, limit],
    queryFn: async () => {
...
export function useIdleKernels(threshold_seconds: number = 300) {
  ...
  return useQuery<IdleKernelsResponse>({
    queryKey: ['e8', 'idleKernels', threshold_seconds],
    queryFn: async () => {
      const response = await fetch(`${API_ROUTES.olympus.e8.idleKernels}?threshold=${threshold_seconds}`, {
        credentials: 'include',
      });
      if (!response.ok) {
...
export function useDeleteKernel() {
  ...
  return useMutation<DeleteKernelResponse, Error, { kernelId: string }>({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['e8', 'idleKernels'] });
      queryClient.invalidateQueries({ queryKey: E8_KEYS.status });
    },
  });
}
