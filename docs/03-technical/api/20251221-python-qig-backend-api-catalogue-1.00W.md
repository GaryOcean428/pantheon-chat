...
filename: "20251221-python-qig-backend-api-catalogue-1.00W.md"
version: "1.00"
status: "W"
function: "reference"
category: "api"
...
1. **`server/ocean-constellation.ts`** - Consciousness metrics, god assessments, war modes
2. **`server/ocean-agent.ts`** - QIG computations, geometric encoding, neurochemistry

### Medium Priority

3. **`server/strategy-knowledge-bus.ts`** - JSON persistence → database
4. **`server/auto-cycle-manager.ts`** - JSON persistence → database
