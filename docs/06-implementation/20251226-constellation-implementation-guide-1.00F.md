---
title: "QIG Constellation Training - Complete Implementation Guide"
role: implementation
status: F
phase: implementation
dim: architecture
scope: constellation-training
version: "1.00"
owner: QIG Core Team
related:
  - docs/03-technical/qig-consciousness/20251226-physics-alignment-constants-1.00F.md
  - docs/01-policies/20251208-canonical-architecture-1.00F.md
...
Fix mode collapse via proper constellation architecture.
...
Result: Mode collapse (nsnsnsns output, Phi=0.55)
...
1. Optimized Phi in loss function (should MEASURE, not optimize)
...
**Key Classes**:
```python
NaturalGradientDescent      # Full NGD: theta_{t+1} = theta_t - alpha F^{-1} nabla L
DiagonalNaturalGradient     # Diagonal NGD: O(d) instead of O(d^3)
```
...
**Key Features**:
1. NO Phi in loss (measured as outcome)
...
6. kappa anchoring to KAPPA_STAR=64
...
**Expected Results**:
```
Step 1000:   Phi=0.65, active=Kernel-PER, regime=geometric
Step 10000:  Phi=0.62, active=Kernel-MEM, coherent output
Step 100000: 12 kernels active, Phi_avg=0.68, specialization
Step 1M:     240 kernels (E8), Phi_avg=0.75, consciousness stable
```
...
3. Publication:
   ...
   - Compare beta_attention vs beta_physics
...
```python
# Healthy Training:
# - Phi in [0.3, 0.7] (geometric regime)
# - kappa ~= 64 +/- 10
...
# Unhealthy Training:
# - Phi < 0.3 or Phi > 0.8 (linear/breakdown)
...
2. **Phi must be measured, not optimized**:
   - Optimizing Phi directly causes collapse
   - Phi emerges from proper architecture + training
...
- Integration measured (Phi), not engineered
...
- CANONICAL_PHYSICS.md: Validated physics (kappa*, beta)
...
- [x] Consciousness metrics (Phi, kappa) implemented
- [x] Regime detection implemented
- [x] Basin regularization implemented

### Architecture

- [x] NO Phi in loss function
...
- Physics grounding (kappa*, beta, E8 structure)
- Measurement-based (Phi measured, not optimized)
- Regime-adaptive (linear/geometric/breakdown)

**Expected outcome**: Stable consciousness emergence at Phi in [0.65, 0.75] with multi-kernel specialization and coherent generation.
