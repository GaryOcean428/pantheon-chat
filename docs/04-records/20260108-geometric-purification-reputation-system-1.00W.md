...
BEGIN
    ...
    LEFT JOIN tokenizer_vocabulary tv ON tv.token = lw.word
...
vocab = coordizer.load_tokenizer_vocabulary()
...
**Current State:** Embeddings set to NULL in tokenizer_vocabulary, coordizer computes on-demand.
...
SELECT token FROM tokenizer_vocabulary 
WHERE basin_embedding IS NULL 
  AND source_type = 'learned'
LIMIT 1000;  -- Process in batches

-- Coordizer computes Fisher-Rao embeddings
-- UPDATE tokenizer_vocabulary SET basin_embedding = ? WHERE token = ?
```
