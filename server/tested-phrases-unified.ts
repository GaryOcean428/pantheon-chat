...
export class TestedPhrasesUnified {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (registry && db) {
        ...
        console.log(
          `[TestedPhrasesUnified] ✅ Hydrated with ${memoryCache.size} historical phrases from PostgreSQL`
        );
      } else {
      ...
      console.error(
        "[TestedPhrasesUnified] ⚠️ Failed to hydrate from PostgreSQL:",
        error
      );
  ...
  has(phrase: string): boolean {
    if (!this.isHydrated) {
      console.warn(
        "[TestedPhrasesUnified] has() called before hydration - returning false"
      );
      return false;
    }
  ...
  async recordTested(
    ...
    if (registry) {
      ...
      if (existing) {
        ...
        console.warn(
          `[TestedPhrasesUnified] WASTE DETECTED: Phrase "${phrase.substring(
            0,
            30
          )}..." re-tested (${existing.retestCount} times)`
        );
      } else {
        // New phrase
  ...
  async getBalanceHits(): Promise<TestedPhrase[]> {
    ...
    if (registry) {
      ...
      // In-memory fallback
      return Array.from(memoryRegistry.values()).filter(
        (p) => (p.balanceSats || 0) > 0
      );
    }
  }
  ...
  async getStats(): Promise<{
    ...
    if (registry) {
      ...
      return {
        ...
        uniqueAddresses: new Set(cached.map((p) => p.address).filter(Boolean))
          .size,
