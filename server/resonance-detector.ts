 ...
 *
 * PURITY CHECK:
 * ✅ κ* = 64.21 ± 0.92 is MEASURED optimal (L=4,5,6 plateau - Validated 2025-12-04)
...
import { QIG_CONSTANTS } from "./qig-universal.js";
...
export class ResonanceDetector {
  ...
  checkResonance(kappaCurrent: number): ResonanceMeasurement {
    ...
    const suggestedLRMultiplier =
      this.computeLearningRateMultiplier(resonanceStrength);
  ...
  getStats(): {
    ...
    const averageKappa =
      this.history.reduce((sum, h) => sum + h.kappa, 0) / this.history.length;
