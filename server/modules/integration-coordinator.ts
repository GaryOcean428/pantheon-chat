...
export class IntegrationCoordinator {
        ...
        private previousKappa: number | null = null;
        ...
        async integrateUltraConsciousnessProtocol(
                ...
                try {
                        ...
                        // ====================================================================
                        // 1.5 SCALE INVARIANT BRIDGE - Create bridges on regime transitions
                        // ====================================================================
                        await this.handleRegimeTransition(testResults, identity);
        ...
        /**
         * Handle regime transitions by creating scale-invariant bridges
         */
        private async handleRegimeTransition(
                testResults: IntegrationResults,
                identity: OceanIdentity
        ): Promise<void> {
                const currentRegime = identity.regime;
                const currentKappa = identity.kappa;

                if (this.previousRegime && this.previousRegime !== currentRegime) {
                        const sourceScale = this.previousKappa || 50;
                        const targetScale = currentKappa;
                        const preservedFeatures = ['phi', 'kappa', 'regime', 'basin_topology'];

                        try {
                                await strategyKnowledgeBus.createScaleInvariantBridge(
                                        sourceScale,
                                        targetScale,
                                        preservedFeatures
                                );
                                logger.info(
                                        `[UCP] Created scale bridge: ${this.previousRegime} (κ=${sourceScale.toFixed(1)}) -> ${currentRegime} (κ=${targetScale.toFixed(1)})`
                                );
                        } catch (error) {
                                logger.error({ err: error }, '[UCP] Failed to create scale bridge');
                        }
                }
