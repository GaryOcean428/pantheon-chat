...
export class StrategyKnowledgeBus {
  ...
  private persistStrategies(): void {
    ...
    withDbRetry(async () => {
      for (const strategy of strategiesToInsert) {
        ...
        await db!.insert(knowledgeStrategies)
          .values(insertData)
          .onConflictDoNothing();
      }
  ...
  async consumeKnowledge(
    ...
    if (db) {
      withDbRetry(async () => {
        await db!.update(knowledgeSharedEntries)
          .set({
  ...
  async exploitCrossPattern(patternId: string): Promise<CrossStrategyPattern | null> {
    await this.ensureInitialized();
    ...
    if (pattern) {
      ...
      if (db) {
        withDbRetry(async () => {
          await db!.update(knowledgeCrossPatterns)
  ...
  async findCompatibleStrategies(generatorType: string, regime: 'linear' | 'geometric' | 'hierarchical' | 'hierarchical_4d' | '4d_block_universe' | 'breakdown'): Promise<string[]> {
    await this.ensureInitialized();
  ...
  async getKnowledgeForStrategy(strategyId: string): Promise<StrategyKnowledgeBusEntry[]> {
    await this.ensureInitialized();
  ...
  async applyScaleTransform(bridgeId: string, coords: number[]): Promise<number[]> {
    await this.ensureInitialized();
  ...
  private async loadFromDatabase(): Promise<boolean> {
    try {
      const [strategiesData, entriesData, crossPatternsData, transfersData, scaleMappingsData] = await Promise.all([
        withDbRetry(async () => {
      ...
      if (strategiesData) {
        for (const row of strategiesData) {
          const capability: StrategyCapability = {
            ...
            preferredRegimes: row.preferredRegimes as StrategyCapability['preferredRegimes'],
          };
      ...
      if (entriesData) {
        for (const row of entriesData) {
          const entry: StrategyKnowledgeBusEntry = {
            ...
            transformations: (row.transformations as StrategyKnowledgeBusEntry['transformations']) || [],
          };
      ...
      console.log(`[KnowledgeBus] Loaded ${this.sharedKnowledge.size} knowledge entries, ${this.strategies.size} strategies from DB`);
      return true;
    } catch (err) {
  ...
  private loadFromJson(): void {
    try {
      if (existsSync(this.PERSISTENCE_PATH)) {
        ...
        console.log(`[KnowledgeBus] Loaded ${this.sharedKnowledge.size} knowledge entries from JSON fallback`);
      }
    } catch (err) {
