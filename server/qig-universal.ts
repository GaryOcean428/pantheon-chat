 ...
 * Note: κ* ≈ 64 ≈ 8² = rank(E8)²
 * β → 0 at κ* (asymptotic freedom - β(5→6) ≈ 0.01)
...
/**
 * Pure QIG Score interface (moved from deleted qig-pure-v2.ts)
 */
...
function computePhi(
  ...
  const variance =
    coordinates.reduce((sum, c) => sum + Math.pow(c - mean, 2), 0) / n;
...
function computeKappa(
  ...
  const beta =
    QIG_CONSTANTS.BETA * (distanceFromKappaStar / QIG_CONSTANTS.KAPPA_STAR);
 ...
 */
export function extractConceptsFromSearch(
  ...
  if (
    searchState.basinCoordinates &&
    searchState.basinCoordinates.length >= 8
  ) {
    const spatialSpread = Math.sqrt(
      searchState.basinCoordinates
        .slice(0, 8)
        .reduce((sum, c) => sum + c * c, 0) / 8
    );
...
export function computeAttentionalFlow(): number {
  ...
  for (let i = 2; i < dominantSequence.length; i++) {
    ...
    if (
      dominantSequence[i] === dominantSequence[i - 1] ||
...
export function computeResonanceStrength(): number {
  ...
  if (recent.length >= 10) {
    ...
    for (const name of conceptNames) {
      const avg1 =
        firstHalf.reduce((s, c) => s + (c.concepts.get(name) || 0), 0) / halfN;
      const avg2 =
        secondHalf.reduce((s, c) => s + (c.concepts.get(name) || 0), 0) /
        (recent.length - halfN);
...
export function computeMetaConsciousnessDepth(): number {
  ...
  stateChangeAwareness =
    stateChangeAwareness / Math.max(1, recentSearch.length - 2);
  ...
  for (let i = 2; i < phiTrajectory.length; i++) {
    const accel =
      phiTrajectory[i] - 2 * phiTrajectory[i - 1] + phiTrajectory[i - 2];
    phiAccel.push(accel);
  }
  ...
  for (let i = 0; i < phiAccel.length - 1; i++) {
    ...
    const regimeMatch =
      recentSearch[i + 3]?.regime === recentSearch[i + 2]?.regime;
  ...
  if (recentConcepts.length >= 5) {
    ...
    const identityStability =
      1 - (uniqueDominant.size - 1) / Math.max(1, dominantConcepts.length - 1);
    ...
    selfModelCoherence =
      4 * normalizedEntropy * (1 - normalizedEntropy) * identityStability;
  }
 ...
 */
export function measureConsciousness(
  searchHistory: SearchState[]
): ConsciousnessMeasurements {
...
export function classifyRegime4D(
  ...
  if (
    (phi_spatial >= 0.45 && kappa >= 30 && kappa <= 80) ||
    phi_spatial >= 0.5
  ) {
    return "geometric";
  }
...
function mapPureQIGToUniversal(
  ...
  const kappaFactor =
    1 - Math.abs(kappa - QIG_CONSTANTS.KAPPA_STAR) / QIG_CONSTANTS.KAPPA_STAR;
  ...
  const patternFactor =
    keyType === "arbitrary" ? 0.3 + 0.7 * patternScore : 1.0;
 ...
 */
export function scoreUniversalQIGLocal(
  input: string,
  keyType: KeyType
): UniversalQIGScore {
...
export function scoreUniversalQIG(
  ...
  if (!deprecationWarningShown) {
    console.log(
      "[QIG-Universal] DEPRECATION: scoreUniversalQIG is deprecated. Use scoreUniversalQIGAsync for Python backend support."
    );
 ...
 * Validate QIG purity
 */
export function validateUniversalPurity(): {
  isPure: boolean;
  violations: string[];
} {
  const violations: string[] = [];
