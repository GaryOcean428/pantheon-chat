...
-- ============================================================================
-- Purpose: Consolidate learned_words data into coordizer_vocabulary
-- Project: pantheon-chat
-- Date: 2026-01-12
-- Related: VOCABULARY_CONSOLIDATION_PLAN.md
-- 
-- This migration adds generation capabilities to coordizer_vocabulary by:
-- 1. Adding new columns for generation filtering (token_role, is_real_word, qfi_score)
-- 2. Migrating all learned_words data into coordizer_vocabulary
...
BEGIN
    ...
    IF EXISTS (
        ...
        EXECUTE (
            SELECT 'ALTER TABLE coordizer_vocabulary DROP CONSTRAINT IF EXISTS ' || conname
            FROM pg_constraint c
            JOIN pg_attribute a ON a.attnum = ANY(c.conkey) AND a.attrelid = c.conrelid
            WHERE c.conrelid = 'coordizer_vocabulary'::regclass
...
BEGIN
    ...
    ELSE
        ...
        UPDATE coordizer_vocabulary SET token_role = 'encoding' WHERE token_role IN ('word', 'subword', 'special') OR token_role IS NULL;
        -- Update default
        ALTER TABLE coordizer_vocabulary ALTER COLUMN token_role SET DEFAULT 'encoding';
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'coordizer_vocabulary_token_role_check'
    ) THEN
        ALTER TABLE coordizer_vocabulary 
        ADD CONSTRAINT coordizer_vocabulary_token_role_check 
...
BEGIN
    ...
    ELSE
        ...
        ALTER TABLE coordizer_vocabulary ALTER COLUMN phrase_category SET DEFAULT 'unknown';
        UPDATE coordizer_vocabulary SET phrase_category = 'unknown' WHERE phrase_category IS NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'coordizer_vocabulary' AND column_name = 'is_real_word'
    ) THEN
        ALTER TABLE coordizer_vocabulary 
        ADD COLUMN is_real_word BOOLEAN DEFAULT FALSE;
        RAISE NOTICE 'Added is_real_word column to coordizer_vocabulary';
    END IF;
END $$;
...
-- 2.1 For words in BOTH tables: Update coordizer_vocabulary with learned_words data
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'learned_words') THEN
        UPDATE coordizer_vocabulary tv
        SET 
            token_role = 'both',
...
-- 2.2 For words ONLY in learned_words: Insert into coordizer_vocabulary with token_role='generation'
...
BEGIN
    ...
    SELECT COALESCE(MAX(token_id), 0) INTO max_token_id FROM coordizer_vocabulary;
    ...
    IF has_basin_embedding THEN
        ...
        FROM learned_words lw
        WHERE NOT EXISTS (
            SELECT 1 FROM coordizer_vocabulary tv 
            WHERE LOWER(tv.token) = LOWER(lw.word)
        )
...
CREATE INDEX idx_tokenizer_vocab_generation 
    ON coordizer_vocabulary(token_role, phrase_category) 
...
CREATE INDEX idx_tokenizer_vocab_real_word 
    ON coordizer_vocabulary(is_real_word) 
...
CREATE INDEX idx_tokenizer_vocab_qfi 
    ON coordizer_vocabulary(qfi_score DESC NULLS LAST)
...
CREATE INDEX idx_tokenizer_vocab_gen_phi 
    ON coordizer_vocabulary(phi_score DESC, frequency DESC)
...
DECLARE
    ...
    real_word_count INT;
BEGIN
    SELECT COUNT(*) INTO encoding_count FROM coordizer_vocabulary WHERE token_role = 'encoding';
    SELECT COUNT(*) INTO generation_count FROM coordizer_vocabulary WHERE token_role = 'generation';
    SELECT COUNT(*) INTO both_count FROM coordizer_vocabulary WHERE token_role = 'both';
    SELECT COUNT(*) INTO real_word_count FROM coordizer_vocabulary WHERE is_real_word = TRUE;
    ...
    RAISE NOTICE '============================================';
    RAISE NOTICE 'coordizer_vocabulary breakdown:';
    ...
    IF (generation_count + both_count) >= learned_count THEN
        RAISE NOTICE 'VERIFICATION PASSED: All learned_words migrated to coordizer_vocabulary';
    ELSE
        RAISE WARNING 'VERIFICATION NOTICE: Some learned_words may not have migrated (% in learned_words, % generation-ready in coordizer_vocabulary)', 
...
-- 1. coordizer_vocabulary now serves BOTH encoding AND generation
...
-- coordizer_vocabulary is now the single source of truth.
-- ============================================================================
